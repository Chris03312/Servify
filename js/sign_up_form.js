  ...
  Array.from(forms).forEach(form => {
    form.addEventListener('submit', event => {
      ...
      if (!allFieldsValid || !passwordsMatch || !passwordCriteriaValid) {
        ...
        // Prevent default submission, show modal, and trigger submission later
        event.preventDefault(); // Prevent actual form submission initially
        const successModal = new bootstrap.Modal(document.getElementById('SuccessModal'));
        successModal.show();

        // Add listener for "Done" button
        const doneButton = document.querySelector('#SuccessModal button[data-bs-dismiss="modal"]');
        doneButton.addEventListener(
          'click',
          () => {
            form.submit(); // Programmatically submit the form to the specified action URL
          },
          { once: true } // Ensure the listener triggers only once
        );
      }
