...
<body>
    ...
    <main class="container-fluid p-3">
        ...
        <div class="container-fluid">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="d-flex flex-column justify-content-center align-items-center gap-2">
                                ...
                                <div class="d-flex flex-sm-row flex-column justify-content-center align-items-center gap-2">
                                    ...
                                    <button type="button" class="btn btn-danger px-5" data-bs-toggle="modal" data-bs-target="#questionOneNoModal">No</button>
                                </div>
                            </div>
            ...
            <!-- IF QUESTION #1 IS NO -->
            <div class="modal fade" id="questionOneNoModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="questionOneNoModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header bg-primary p-2"></div>
                        <div class="modal-body">
                            ...
                            <div class="d-flex flex-column justify-content-center align-items-center gap-2">
                                <img src="../../img/question-mark.png" alt="Question Mark Icon" class="img-fluid" style="width: 80px;">
                                <p class="text-center">Are you sure you want to <strong>stop</strong> volunteering?</p>
                                <div class="d-flex flex-sm-row flex-column justify-content-center align-items-center gap-2">
                                    <button type="button" class="btn btn-primary px-5" data-bs-toggle="modal" data-bs-target="#volunteerParticipationModal">Yes</button>
                                    <button type="button" class="btn btn-danger px-5" data-bs-toggle="modal" data-bs-target="#volunteerContinuationModal">No</button>
                                </div>
                            </div>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="d-flex flex-column justify-content-center align-items-center gap-2">
                                ...
                                <div class="d-flex flex-sm-row flex-column justify-content-center align-items-center gap-2">
                                    <button type="button" class="btn btn-primary px-5" data-bs-dismiss="modal" id="agreeGuidelinesBtn">Yes</button>
                                    <button type="button" class="btn btn-danger px-5" data-bs-toggle="modal" data-bs-target="#guidelinesAgreementModalNo">No</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <!-- IF NO IN FOURTH MODAL -->
            <div class="modal fade" id="guidelinesAgreementModalNo" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="guidelinesAgreementModalNoLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header bg-primary p-2"></div>
                        <div class="modal-body">
                            ...
                            <div class="d-flex flex-column justify-content-center align-items-center gap-2">
                                <img src="../../img/warning.png" alt="Warning Icon" class="img-fluid" style="width: 80px;">
                                <p class="text-center">You must agree to the organization's guidelines to continue.</p>
                                <button type="button" class="btn btn-primary px-5" data-bs-dismiss="modal" id="guidelinesOkayBtn">Okay</button>
        ...
        <!-- FORM -->
        <div class="container-fluid bg-light p-3 d-none" id="formContainer">
                ...
                <div class="tab-content border-start border-end border-bottom" id="myTabContent"
                    ...
                    <div class="tab-pane fade show active p-3" id="home-tab-pane" role="tabpanel"
                        ...
                        <div class="row">
                            ...
                            <div class="my-5 d-flex flex-row justify-content-end align-items-center gap-2">
                                <button class="btn px-4" id="cancelBtn">Cancel</button>
                                <button class="btn btn-primary px-4" id="submitBtn">Submit</button>
                            </div>
    ...
    // Fetch the latest renewal period
    $query = "SELECT renewal_from, renewal_to FROM events LIMIT 1";
    $stmt = Database::getConnection()->prepare($query);
    $stmt->execute();
    $renewal = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($renewal) {
        // Convert to readable format: "March 8, 2025"
        $renewalFrom = !empty($renewal['renewal_from']) ? date("F j, Y", strtotime($renewal['renewal_from'])) : null;
        $renewalTo = !empty($renewal['renewal_to']) ? date("F j, Y", strtotime($renewal['renewal_to'])) : null;
    } else {
        $renewalFrom = null;
        $renewalTo = null;
    }
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            const formattedRenewalFrom = "<?php echo $renewalFrom; ?>";
            const formattedRenewalTo = "<?php echo $renewalTo; ?>";

            const renewalFrom = "<?php echo date('Y-m-d', strtotime($renewalFrom)); ?>";
            const renewalTo = "<?php echo date('Y-m-d', strtotime($renewalTo)); ?>";
            const today = new Date().toISOString().split("T")[0];

            const formContainer = document.getElementById("formContainer");

            // Check if user already agreed
            if (localStorage.getItem("agreedToGuidelines")) {
                formContainer.classList.remove("d-none"); // Show form
            } else {
                const modalId = (today >= renewalFrom && today <= renewalTo) ? "volunteerContinuationModal" : "renewalModal";
                new bootstrap.Modal(document.getElementById(modalId)).show();
            }

            // Handle "Yes" button click to show form and store agreement
            document.getElementById("agreeGuidelinesBtn").addEventListener("click", function() {
                localStorage.setItem("agreedToGuidelines", "true");
                formContainer.classList.remove("d-none"); // Show form
            });

            // Handle other button redirects
            document.querySelectorAll("#NotRenewalOK, #guidelinesOkayBtn").forEach(button => {
                button.addEventListener("click", function() {
                    const token = new URLSearchParams(window.location.search).get("token");
                    window.location.href = `/volunteer_dashboard?token=${encodeURIComponent(token)}`;
                });
            });
        });
    </script>
